...
function node_html(n, lane) {
	if (n.kind === "start" || n.kind === "end") {
		...
		return `<div class="gpm-node gpm-pill" id="gpm-${n.id}"
			style="grid-column:${n.lane};grid-row:${n.row};background:${bg}">${frappe.utils.escape_html(n.title)}</div>`;
	}
	if (n.kind === "decision") {
	...
	const links = (n.links || []).map(([label, dt]) => {
		...
		return `<span class="gpm-chip">
			<a class="gpm-open" href="/app/${s}" title="${__("Open list")}">${frappe.utils.escape_html(label)}</a>
			<a class="gpm-new" href="/app/${s}/new" title="${__("Create new")}">＋</a>
		</span>`;
	}).join("");
	const gate = n.gate
		? `<span class="gpm-gate" tabindex="0">⚠<span class="gpm-tip">${frappe.utils.escape_html(n.gate)}</span></span>`
		: "";
	return `<div class="gpm-node gpm-step" id="gpm-${n.id}"
		...
		<div class="gpm-step-title">
			...
			${frappe.utils.escape_html(n.title)}${gate}
		</div>
		<div class="gpm-step-desc">${frappe.utils.escape_html(n.desc || "")}</div>
